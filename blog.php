    ...
    <section id="about1" class="about about-1 sec-space-top">
        <div class="container">
            <div class="row">
                ...
                <?php  $count =1;
                              $sql = "SELECT `id`,`title`,`blog_url`,`short_description`,`image`,`published_date`,`meta_title`,`meta_description`,`meta_keywords` FROM `blogs` where status = 1 ORDER BY `published_date` DESC ";
                              ...
                              $short_description = mb_convert_encoding($row['short_description'], 'UTF-8', 'UTF-8');
							   ...
							   $title = removeHtml($row['title']);
                               $short_description = removeHtml($row['short_description']);
